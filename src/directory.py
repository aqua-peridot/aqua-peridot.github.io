...
def manage_dirs():
    ...
    OPT_COMPILED_DIR = os.path.join(WORKSPACE_DIR, 'opt-compiled.json')
    if not os.path.exists(OPT_COMPILED_DIR):
        with open(OPT_COMPILED_DIR, 'w') as f:
            json.dump({}, f, indent=4)
    ...
    DIRS.update({
        ...
        'OPT_COMPILED_DIR': OPT_COMPILED_DIR,
